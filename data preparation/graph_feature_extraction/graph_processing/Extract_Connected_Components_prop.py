...
end_time = time.time()
gp.shortest_path_lengths_lcc_calc_time = end_time - start_time
...
sp_info = {'shortest paths lengths min': min_sp, 'shortest paths lengths max': max_sp, 'shortest paths lengths mean': mean_sp, 'shortest paths lengths median': median_sp, 'shortest paths lengths var': var_sp, 'shortest paths lengths distr': distr_sp, 'shortest_path_lengths_calc_time': gp.shortest_path_lengths_lcc_calc_time}

with open(feature_folder + graph_file_name + '_sp_lcc.json', 'w') as fout:
...
min_fc = min(fc_list)
max_fc = max(fc_list)
mean_fc = np.mean(fc_list)
var_fc = np.var(fc_list)
median_fc = np.median(fc_list)
#distr_fc = calc_cdf(fc_list)

fc_info = {'farness centrality min': min_fc, 'farness centrality max': max_fc, 'farness centrality mean': mean_fc, 'farness centrality median': median_fc, 'farness centrality var': var_fc}

with open(feature_folder + graph_file_name + '_farness_LCC.json', 'w') as fout:
    json.dump(fc_info, fout)

# ecc
min_ecc = min(ecc_list)
max_ecc = max(ecc_list)
mean_ecc = np.mean(ecc_list)
var_ecc = np.var(ecc_list)
median_ecc = np.median(ecc_list)
#distr_fc = calc_cdf(fc_list)

ecc_info = {'eccentricity centrality min': min_ecc, 'eccentricity centrality max': max_ecc, 'eccentricity centrality mean': mean_ecc, 'eccentricity centrality median': median_ecc, 'eccentricity centrality var': var_ecc}

with open(feature_folder + graph_file_name +'_ecc_LCC.json', 'w') as fout:
    json.dump(ecc_info, fout)
...
sp_info = {'shortest paths lengths min': min_sp, 'shortest paths lengths max': max_sp, 'shortest paths lengths mean': mean_sp, 'shortest paths lengths median': median_sp, 'shortest paths lengths var': var_sp, 'shortest paths lengths distr': distr_sp}
